...
# CtagsArgs extra arguments for ctags command
...
class AutoTag:
   ...
   def __init__(self):
      ...
      self.ctags_args = vim_global("CtagsArgs")
   ...
   def updateTagsFile(self, tagsDir, tagsFile, sources):
      ...
      if self.tags_file:
         cmd = "%s -f %s -a %s" % (self.ctags_cmd, self.tags_file, self.ctags_args)
      else:
         cmd = "%s -a %s" % (self.ctags_cmd, self.ctags_args)
